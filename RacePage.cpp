...
void RacePage::race() {
    ...
    if (raceTimer.isItTime()){
        ...
        if (!countDown) {
			...
			if (!opFinished) {
                simRace();
				//infoString = "Opponent Time : " + to_string(raceStart.getElapsedTime().asSeconds());
			}
...
void RacePage::simRace() {
    ...
    //if (opGear > 1) {
    //	if (opSpeed < opCar.getTopSpeed(opGear - 1)) {
    //		 printf("STALL!\n");
    //	}
    ...
    if (opGear > 0) {
      ...
      //  cout << "A: " << temp << endl;
        ...
        if ((opSpeed < maxSpeed)) {
            opSpeed = opSpeed + speedInc;// (temp * dt.getElapsedTime().asSeconds());
...
void RacePage::opRPMS() {
    ...
    double dif=0, di2 = 0;
    double maxSpeed = .1;
    double gRatio = opCar->getDrivetrain()->getTransmistion()->gearRatio[opGear];
    double torq = opCar->calcToque(opRpms, opGear);
  ...
  //  cout <<"D2: " <<di2 << endl;
    ...
    //if (opGear > opCar.getMaxGear()) {
    //    opGear = opGear > opCar.getMaxGear();
    //} 
    cout << "opGear: " << opGear << endl;
    cout << "OP rpm: " << opRpms << endl;
}
